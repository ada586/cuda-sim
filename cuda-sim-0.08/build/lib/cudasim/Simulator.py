...
class Simulator(object):
    ...
    def _getOptimalGPUParam(self, parameters, compiledRunMethod = None):
        ...
        maxThreads = min(old_div(driver.Device(self._device).max_registers_per_block,compiledRunMethod.num_regs), self._MAXTHREADSPERBLOCK)
        
        maxWarps = old_div(maxThreads, warp_size)
        ...
        if (len(parameters)*self._beta%threads == 0):
            blocks = old_div(len(parameters)*self._beta,threads)
        else:
            blocks = old_div(len(parameters)*self._beta,threads) + 1
    ...
    def run(self, parameters, initValues, timing=True, info=False):
        ...
        for i in range(runs):
            ...
            runParameters = parameters[old_div(minIndex,self._beta):old_div(maxIndex,self._beta)]
