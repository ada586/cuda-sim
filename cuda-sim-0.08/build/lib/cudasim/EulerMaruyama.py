...
class EulerMaruyama(sim.Simulator):
    ...
    def _compile(self, step_code):         
        ...
        freeSharedMemory = totalSharedMemory - old_div(maxThreads,self._warp_size*self._state_words * 4)
        
        # assuming maximum number of threads (should be per MP)
        maxParameters = old_div(freeSharedMemory, maxThreads / 4)
    ...
    def _runSimulation(self, parameters, initValues, blocks, threads):
        ...
        param = np.zeros((old_div(totalThreads,self._beta) + 1,self._parameterNumber),dtype=np.float32)
        try:
        ...
        if(not self._putIntoShared):
            ...
            sim.copy2D_host_to_array(ary, param , self._parameterNumber*4, old_div(totalThreads,self._beta) + 1)
            self._param_tex.set_array(ary)
            sharedMemoryParameters = 0
        else:
            # parameter shared Mem
            sharedMemoryParameters = self._parameterNumber * (old_div(threads,self._beta)+2) * 4
        
        sharedMemoryPerBlockForRNG = old_div(threads,self._warp_size*self._state_words * 4)
        sharedTot = sharedMemoryPerBlockForRNG + sharedMemoryParameters
        
        if(self._putIntoShared):
            parametersInput = np.zeros(old_div(self._parameterNumber*totalThreads,self._beta), dtype=np.float32)
        ...
        seeds = np.zeros(old_div(totalThreads,self._warp_size*self._state_words), dtype=np.uint32)
